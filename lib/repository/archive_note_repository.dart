...
class ArchiveNoteRepository {
  ...
  ArchiveNoteRepository(this._noteRepository, this._fireStoreDb);
  ...
  void saveNoteToArchive(NoteItem noteItem) async {
    final reference = await _fireStoreDb.saveToCollection(_collectionPath, noteItem.toJson());
  ...
  void restoreNote(NoteItem noteItem) async {
    final reference = await _fireStoreDb.saveToCollection(_notePath, noteItem.toJson());
    reference.update({"referenceId": reference.id});
    _fireStoreDb.deleteCollection(_collectionPath, noteItem.referenceId);
  }
}
